...
export const Table: FunctionComponent<ITableProps> = ({ data, selectedQuote }) => {
  ...
  const renderQuotesChange = (high: number, markPrice: number) => {
    const priceChange = getPriceChange(high, markPrice);
    if (priceChange > 0) {
  ...
  return (
    <TableSection>
      ...
      <tbody>
        {data &&
          data.map(({ displayName, high, markPrice }: IQuote, index: number) => (
            <StyledTr key={index + displayName} onClick={() => handleQuotationClick(displayName)}>
              ...
              <StyledTd
                style={{
                  ...
                  color: markPrice > high ? green : markPrice && markPrice < high ? danger : '',
                }}
              >
